import { getMapByString } from "../../../common/DateUtils.js";
import {
  JUEJIN_POST_URL,
  JUEJIN_USER_URL,
} from "../../../common/base.js";
...
// 专栏模块处理
export function getArticleColumnList(columnMap) {
...
// 统计模块
export function getCountArticles(userArticleList, articleMap) {
  // 添加统计数据操作
  var countMap = {
    // 添加总文章数
...
// 解析文章列表
export function getArticleMap(articleList) {
...
// 解析、处理文章
export function getArticleInfo(article) {
...
export function unique(arr) {
  ...
  arr = arr.filter(
    (item) =>
      !res.has(item.article_info.ctime) && res.set(item.article_info.ctime, 1),
  ); // 数组去重
