...
const getREADME = (user_id, cover_png) => {
  ...
  var template = readFileSync(README_TEMPLATE_PATH, "utf-8");
  template = template
    .replaceAll("{{link}}", JUEJIN_USER_URL + user_id)
    .replaceAll("{{cover_png}}", cover_png);
  return template;
};

// 复制本地的问题
const downloadLocalFile = (url, filePath) => {
  readFile(url, function (err, originBuffer) {
    // 读取文件路径（图片）
    writeFileSync(filePath, originBuffer, function (err) {
      // 把 buffer 写入到文件
      if (err) {
        console.log(err);
      }
...
const getConfig = (userBean) => {
  ...
  var template = readFileSync(CONFIG_TEMPLATE_PATH, "utf-8");
  template = template
...
// 根据个人信息生成 Vite Press 配置
export function getVuepressInfo(userBean, articleMap) {
  var vuepressParams = {};
  ...
  for (var yy of yyList) {
    ...
    time_sort_list_str += time_sort_template
      .replace("{{year}}", yy)
      .replace("{{year}}", yy);
  }
  time_sort_list_str += "]";
