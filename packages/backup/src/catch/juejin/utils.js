...
import { post, get, sleep } from "../../../common/Http.js";

// 文章列表信息请求（post 请求
export const userArtMapPost = async (userId) => {
  console.log("开始-获取个人所有文章");
  // 获取个人所有文章
  var userArticleList = await juejinCommonPost(
    JUEJIN_BASE.ARTICLE_API,
  ...
  var columnList = await juejinCommonPost(
    JUEJIN_BASE.COLUMN_LIST_API,
  ...
  for (var i = 0; i < columnList.length; i++) {
    // 获取所需要的信息
    const { column_version } = columnList[i];
    if (!column_version) {
      continue;
    }
    const { title, column_id } = column_version;
    ...
    // 遍历专栏对应的所有文章
    for (
      var columnArticleIndex = 0;
      columnArticleIndex < columnArticleList.length;
      columnArticleIndex++
    ) {
      ...
      var { article_id } = columnArticleList[columnArticleIndex];
      if (!article_id) {
        console.log("空了");
        continue;
      }
      // 已经在专栏里的，进行删除深拷贝的那份
      var removeIndex = noColumnArticleList.findIndex(
...
export const columnArtListPost = async (column_id) => {
  // 获取指定专栏
  return await juejinCommonPost(
    JUEJIN_BASE.COLUMN_API,
    getColumnsParams(column_id),
    1,
  );
};
