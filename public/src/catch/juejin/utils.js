...
const userArtMapPost = async (userId) => {
    ...
    var columnList = await juejinCommonPost(JUEJIN_BASE.COLUMN_LIST_API, getColumsListParams(userId), 1);
...
// 指定专栏文章列表请求（post 请求
const columnArtListPost = async (column_id) => {
    // 获取指定专栏
    return await juejinCommonPost(JUEJIN_BASE.COLUMN_API, getColumnsParams(column_id), 1);
}

const juejinColumnsInfoPost = async (columnId) => {
    var data = {
        column_id: columnId,
    };
...
// 获取专栏列表的请求参数
const getColumsListParams = (user_id) => {
    return {
